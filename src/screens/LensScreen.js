...
function getDerivedInsights(history) {
  ...
  const postRate = Math.round((posted / history.length) * 100);
  const avgSetSize =
    history.reduce((sum, h) => sum + (h.allIds?.length || 1), 0) / history.length;
...
function buildInsightCards(insights) {
  ...
  if (insights.postRate >= 70) {
    cards.push({
      title: 'You trust your eye',
      body: `You've posted ${insights.postRate}% of what you've picked. That's not someone who second-guesses — that's someone who knows what they like.`,
    });
  } else if (insights.postRate >= 40) {
    cards.push({
      title: "You're selective",
      body: "You post about half of what you analyse. You're using Pick the right way — as a filter, not a rubber stamp.",
    });
  } else {
    cards.push({
      title: 'You take your time',
      body: 'You hold back more than you post. That restraint usually shows in the quality of what does make it through.',
    });
  }

  if (insights.avgSetSize >= 4) {
    cards.push({
      title: 'You think in sets',
      body: `You typically bring in ${insights.avgSetSize} photos at once. You want options before you commit — which usually means a better final call.`,
    });
  } else if (insights.avgSetSize <= 2) {
    cards.push({
      title: "You're decisive",
      body: "You usually come in with 1 or 2 photos. You've already done the editing — Pick is just the final call.",
    });
  }

  if (insights.recentPicks >= 3) {
    cards.push({
      title: 'On a roll this week',
      body: `${insights.recentPicks} picks in the last 7 days. Consistency in posting usually shows up in the feed.`,
    });
  }

  cards.push({
    title: 'Your eye is getting sharper',
    body: `${insights.total} picks in. The more you use Pick, the faster you'll know which photo is right before you even run it.`,
  });
...
// Real headline derived from actual stats — no random strings
function buildHeadline(insights) {
  if (insights.postRate >= 70) return 'You back yourself.';
  if (insights.setPicks > insights.soloPicks) return 'You compare before you commit.';
  if (insights.avgSetSize <= 1.5) return "You know what you're bringing in.";
  return 'A read on what your eye keeps reaching for.';
}
...
export default function LensScreen({ navigation }) {
  ...
  return (
    ...
    >
      ...
      <Text style={styles.sub}>{buildHeadline(insights)}</Text>
      ...
      <View style={styles.statsRow}>
        ...
        <View style={styles.stat}>
          ...
          <Text style={styles.statLabel}>Post rate</Text>
        </View>
      </View>

      {/* Photo strip — their actual choices */}
      ...
      {insights.recentPicks === 0 && (
        <View style={styles.nudgeCard}>
          <Text style={styles.nudgeText}>
            No picks this week. Your eye needs exercise.
          </Text>
